...
-- всего оборудования на сервис, пользователей на сервис, процент оборудования на сервис, всего устройств, всего сервисов, контактная почта группы
-- выборка не работает корректно
SELECT DISTINCT
  services.id, services.name AS service_name,
  COUNT(deviсes.id) OVER w_deices AS device_in_service, 
  COUNT(services_users.user_id) OVER w_deices AS users_in_service, 
  COUNT(deviсes.id) OVER w_deices / (SELECT COUNT(*) FROM deviсes) *100 AS '%%',
  services.email AS services_email,
  total_deviсes, total_services
  FROM 
    ...
    ON services_users.services_id = services.id
    LEFT JOIN deviсes
    ON deviсes.services_id = services.id
   WINDOW w_deices AS (PARTITION BY services.id);